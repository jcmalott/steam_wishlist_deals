    ...
    def _build_ui(self) -> gr.Blocks:
        ...
        with gr.Blocks(theme='soft', css=self._load_css()) as ui:
            ...
            def update_games_container(id):
                ...
                search_input.change(self._display_single_game, inputs=[search_input], outputs=[single_game_container, all_games_container, boxes[0], boxes[1],boxes[2], boxes[3],boxes[4], boxes[5], boxes[6]])
        ...
        with gr.Row(elem_classes='container game-row'):
            ...
            with gr.Column(scale=1):
                ...
                with gr.Row(elem_classes='gap'):
                    ...
                    gg_deals_btn = gr.Button('Lowest', scale=1, elem_classes='price_btn', interactive=False)
                    gg_deals_historic_price_box = gr.Textbox(f"{price_display}", show_label=False, container=False, scale=2)
        
        
        steam_btn.click(fn=self._open_url, inputs=[steam_url])
        gg_deals_btn.click(fn=self._open_url, inputs=[gg_deals_url])
        
        return namebox, steam_price_box, steam_url, gg_deals_price_box, gg_deals_historic_price_box, gg_deals_url, imagebox  
